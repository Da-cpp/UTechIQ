...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [birthdate, setBirthdate] = useState<string | null>(null); // 👈 ADD THIS
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      setUser(currentUser);
      setBirthdate(extractBirthdate(currentUser)); // 👈 ADD THIS
...
export function useAuth() {
  ...
  if (!context) throw new Error('useAuth must be wrapped cleanly inside an AuthProvider');
  return context;
}